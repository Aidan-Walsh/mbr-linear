...
from evaluate2 import evaluate_google_mbpp
            ...
            for idx in id_set:
                ...
                if interpolation:
                    ...
                    for grndtruth_idx in ids:
                        ...
                        token_key = token_bleu_selection_function(
                            item, grndtruth_item)
                        token_bleus.append(token_key)

                        char_key = bleu_selection_function(
                            item, grndtruth_item)
                        char_bleus.append(char_key)

                        # if in collab, use this: m(item, grndtruth_item)
                        token_meteor_key = meteor_selection_function(
                            item, grndtruth_item)
                ...
                item['interpolation'] = np.array(
                    # [item['token_bleu'], item['char_bleu'], first_key, item['avg_logprob']])
...
def select_mbpp(args, return_selected=False, return_selector=False, hyper_params=None):
    ...
    if return_selector:
        ...
        result = evaluate_google_mbpp(
            selected, 'data/mbpp/mbpp.jsonl', 'test', gridSearch=True)  # may have to change location
        return result
